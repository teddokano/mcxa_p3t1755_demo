...
float temp_sensor_setting( uint8_t addr, uint8_t config )
{
	...
	i3c.reg_write( addr, P3T1755_REG_Conf, &config, sizeof( config ) );
...
void temp_sensor_reg_dump( uint8_t addr )
{
	...
	i3c.reg_read( addr, P3T1755_REG_Temp,   (uint8_t *)&t, sizeof( t ) );
	i3c.reg_read( addr, P3T1755_REG_Conf,              &c, sizeof( c ) );
	i3c.reg_read( addr, P3T1755_REG_T_LOW,  (uint8_t *)&l, sizeof( l ) );
	i3c.reg_read( addr, P3T1755_REG_T_HIGH, (uint8_t *)&h, sizeof( h ) );
...
{
	uint16_t	tmp;
	i3c.reg_read( targ, reg, (uint8_t *)&tmp, sizeof( tmp ) );
...
void write_temp( uint8_t targ, uint8_t reg, float v )
{
	...
	i3c.reg_write( targ, reg, (uint8_t *)&tmp, sizeof( tmp ) );
}
